...
object MoveGenerator {
    ...
    fun getValidMoves(piece: ChessPiece, board: ChessBoard): List<Move> {
        ...
        return if (piece.ability == Ability.EXTRA_RANGE) {
    ...
    private fun applyExtraRange(piece: ChessPiece, board: ChessBoard, baseMoves: List<Move>): List<Move> {
        ...
        when (piece.type) {
            PieceType.ROOK -> extra.addAll(getSlidingMoves(piece, board, listOf(Pair(-1,0), Pair(1,0), Pair(0,-1), Pair(0,1)), 2))
            PieceType.BISHOP -> extra.addAll(getSlidingMoves(piece, board, listOf(Pair(-1,-1), Pair(-1,1), Pair(1,-1), Pair(1,1)), 2))
            PieceType.KNIGHT -> {
                ...
                extendedOffsets.forEach { (dr, dc) ->
                    ...
                    if (board.isValidPosition(nr, nc)) {
                        ...
                        if (target == null || target.isPlayer != piece.isPlayer) {
                            extra.add(Move(piece.row, piece.col, nr, nc, target))
                        }
                    }
                }
            }
    ...
    private fun getSlidingMoves(piece: ChessPiece, board: ChessBoard, directions: List<Pair<Int,Int>>, maxSteps: Int = ChessBoard.SIZE): List<Move> {
    ...
    private fun getPawnMoves(piece: ChessPiece, board: ChessBoard): List<Move> {
        ...
        if (board.isValidPosition(nr, piece.col) && board.getPiece(nr, piece.col) == null) {
            ...
            if (piece.row == startRow) {
                ...
                if (board.getPiece(nr2, piece.col) == null) {
